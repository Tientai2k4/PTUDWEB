...
using static ShopManager.Areas.Admin.Models.ProductAdmin;
    ...
    public class ProductAdminDAL
    {
        ...
        public bool AddNew(ProductFormAdmin productNew)
        {
            ...
            using (SqlCommand command = new SqlCommand())
            {
                ...
                string query = @"insert into product (title, content, img, price, 
                    rate, createAt, updateAt, categoryId) 
                    values (@title, @content, @img, @price, @rate, @createAt, 
                    @updateAt, @categoryId); ";
